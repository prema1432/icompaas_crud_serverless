...
def sample_user(app):
    with app.app_context():
        user = IcompasUser(email='tesuaaaut@example.com', first_name='Johun', last_name='Doe', password='password')
        db.session.add(user)
        db.session.commit()
...
def test_get_user(app, client, sample_user):
    ...
    assert user_data["first_name"] == "Johun"
    assert user_data["last_name"] == "Doe"
    assert user_data["email"] == "tesuaaaut@example.com"
